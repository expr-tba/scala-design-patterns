  ...
  class MigrationService {
    ...
    def runMigrations(): Unit = {
      ...
      try {
        ...
        insertPeople(
          connection,
          List((1, "Ivan", 26), (2, "Maria", 25), (3, "John", 27))
        )
        insertClasses(
          connection,
          List(
    ...
    private def dropPeopleTable(connection: Connection): Unit = {
      val statement = connection.prepareStatement("DROP TABLE people")
      try {
        statement.executeUpdate()
      } finally {
        statement.close()
    ...
    private def dropClassesTable(connection: Connection): Unit = {
      val statement = connection.prepareStatement("DROP TABLE classes")
      try {
        statement.executeUpdate()
      } finally {
        statement.close()
    ...
    private def dropPeopleToClassesTable(connection: Connection): Unit = {
      val statement = connection.prepareStatement("DROP TABLE people_classes")
      try {
        statement.executeUpdate()
      } finally {
        statement.close()
    ...
    private def signPeopleToClasses(
        connection: Connection,
        peopleToClasses: List[(Int, Int)]
    ): Unit = {
      val statement = connection.prepareStatement(
